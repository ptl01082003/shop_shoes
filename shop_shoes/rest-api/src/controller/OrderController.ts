...
const OrderController = {
  ...
  updateOrder: async (req: Request, res: Response) => {
    try {
      ...
      const mauSac = await Order.findByPk(MauSacId);
      if (!mauSac) {
        return res
          .status(404)
      ...
      const dongSP = await Order.findByPk(DongSPId);
      const kieuDang = await Order.findByPk(KieuDangId);
      const chatLieu = await Order.findByPk(ChatLieuId);
      const xuatXu = await Order.findByPk(XuatXuId);

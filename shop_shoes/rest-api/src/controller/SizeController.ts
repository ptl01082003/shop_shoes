...
const SizeController = {
  ...
  createSize: async (req: Request, res: Response) => {
    try {
      ...
      const address = await Size.create();
  ...
  updateSize: async (req: Request, res: Response) => {
    try {
      ...
      const existingProduct = await Size.findByPk(id);
      if (!existingProduct) {
        return res
          .status(404)
      ...
      const mauSac = await Size.findByPk(MauSacId);
      if (!mauSac) {
        return res
          .status(404)
      ...
      const dongSP = await Size.findByPk(DongSPId);
      const kieuDang = await Size.findByPk(KieuDangId);
      const chatLieu = await Size.findByPk(ChatLieuId);
      const xuatXu = await Size.findByPk(XuatXuId);

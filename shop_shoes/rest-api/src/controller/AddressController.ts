...
const AddressController = {
  ...
  updateAddress: async (req: Request, res: Response) => {
    try {
      ...
      const existingProduct = await Address.findByPk(id);
      if (!existingProduct) {
        return res
          .status(404)
      ...
      const mauSac = await Address.findByPk(MauSacId);
      if (!mauSac) {
        return res
          .status(404)
      ...
      const dongSP = await Address.findByPk(DongSPId);
      const kieuDang = await Address.findByPk(KieuDangId);
      const chatLieu = await Address.findByPk(ChatLieuId);
      const xuatXu = await Address.findByPk(XuatXuId);
